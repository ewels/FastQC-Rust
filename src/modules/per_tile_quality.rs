...
impl PerTileQualityScores {
    ...
    fn calculate(&self) -> Option<TileCalculatedData> {
        ...
        let all_counts: Vec<&QualityCount> = self
        ...
        for tile_means in means.iter().take(tile_numbers.len()) {
            for (avg, &m) in average_qualities_per_group
                .iter_mut()
                .zip(tile_means.iter())
            {
                *avg += m;
...
impl QCModule for PerTileQualityScores {
    fn process_sequence(&mut self, sequence: &Sequence) {
        ...
        let tile = match sequence
            .id
            .split(':')
            .nth(self.split_position as usize)
    ...
    fn raises_error(&self) -> bool {
        ...
        self.calculate()
            .is_some_and(|data| data.max_deviation > threshold)
    }
    ...
    fn ignore_in_report(&self) -> bool {
        ...
        self.ignore_in_report || self.limits.is_ignored("tile") || self.current_length == 0
    }
    ...
    fn chart_image_name(&self) -> Option<&str> {
        Some("per_tile_quality")
    }
    fn chart_alt_text(&self) -> Option<&str> {
        Some("Per tile sequence quality")
    }
