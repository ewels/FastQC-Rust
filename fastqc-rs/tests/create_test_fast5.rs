...
use fastqc_rs::sequence::fast5::Fast5File;
...
fn test_fast5_progress() {
    let mut reader =
        Fast5File::open("tests/data/multi_read.fast5").expect("Failed to open");

    assert_eq!(reader.percent_complete(), 0.0);

    reader.next(); // read 1
    let pct = reader.percent_complete();
    assert!(pct > 0.0 && pct <= 50.0, "Progress should be ~33%, got {}", pct);
