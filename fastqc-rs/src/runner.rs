...
fn process_group(
    ...
    let base_name = strip_extensions(
        &file_display_name.replace("stdin:", ""),
    );
    ...
    report::archive::create_zip_archive(&modules, &file_display_name, &base_name, &zip_path, &html_content, config.svg_output)?;
...
fn strip_extensions(name: &str) -> String {
    ...
    for ext in &[".gz", ".bz2", ".txt", ".fastq", ".fq", ".csfastq", ".sam", ".bam", ".ubam", ".fast5"] {
...
mod tests {
    ...
    fn test_build_file_groups_casava() {
        ...
        let sample_group = groups.iter().find(|g| g.name == "Sample_S1_L001_R1.fastq.gz").unwrap();
        assert_eq!(sample_group.files.len(), 2);

        // Find the Other group
        let other_group = groups.iter().find(|g| g.name == "Other_S2_L001_R1.fastq.gz").unwrap();
