...
impl SequenceLengthDistribution {
    ...
    fn calculate_distribution(&mut self) {
        ...
        let (starting, interval) =
            Self::get_size_distribution(min_len, max_len, self.nogroup);
...
impl SequenceLengthDistribution {
    fn build_chart_svg(&self) -> String {
        ...
        let max_val = computed.graph_counts.iter().cloned().fold(0.0_f64, f64::max);
...
impl QCModule for SequenceLengthDistribution {
    ...
    fn chart_image_name(&self) -> Option<&str> { Some("sequence_length_distribution") }
    fn chart_alt_text(&self) -> Option<&str> { Some("Sequence length distribution") }
