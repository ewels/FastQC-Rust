...
pub trait QCModule: Send {
    ...
    fn chart_image_name(&self) -> Option<&str> { None }
    ...
    fn chart_alt_text(&self) -> Option<&str> { None }
    ...
    fn generate_chart_svg(&self) -> Option<String> { None }
...
pub fn create_modules(config: &FastQCConfig, limits: &Limits) -> Vec<Box<dyn QCModule>> {
    ...
    if limits.is_module_enabled("quality_base") {
        modules.push(Box::new(per_base_quality::PerBaseQualityScores::new(limits, ng, eg)));
    }

    // 3. PerTileQualityScores
    if limits.is_module_enabled("tile") {
        modules.push(Box::new(per_tile_quality::PerTileQualityScores::new(limits, ng, eg)));
    }

    // 4. PerSequenceQualityScores
    if limits.is_module_enabled("quality_sequence") {
        modules.push(Box::new(per_sequence_quality::PerSequenceQualityScores::new(limits)));
    }

    // 5. PerBaseSequenceContent
    if limits.is_module_enabled("sequence") {
        modules.push(Box::new(per_base_sequence_content::PerBaseSequenceContent::new(limits, ng, eg)));
    }
    ...
    if limits.is_module_enabled("sequence_length") {
        modules.push(Box::new(sequence_length_distribution::SequenceLengthDistribution::new(limits, ng)));
    }
