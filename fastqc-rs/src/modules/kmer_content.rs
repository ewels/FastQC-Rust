...
impl KmerContent {
    ...
    fn add_kmer_count(&mut self, position: usize, kmer_length: usize, kmer: &str) -> bool {
        if position >= self.total_kmer_counts.len() {
            ...
            self.total_kmer_counts.resize_with(position + 1, || vec![0u64; self.kmer_size]);
        ...
        for kmer in self.kmers.values() {
            ...
            for (g, group) in groups.iter().enumerate() {
                ...
                for p in lower..=upper {
                    if p < self.total_kmer_counts.len() && kmer_len - 1 < self.total_kmer_counts[p].len() {
                ...
                if total_group_hits as f32 > predicted && total_group_count > 0 {
                    ...
                    binomial_p_values[g] =
                        (p_val * 4.0f64.powi(kmer_len as i32)) as f32;
                }
            }
...
impl QCModule for KmerContent {
    ...
    fn chart_image_name(&self) -> Option<&str> { Some("kmer_profiles") }
    fn chart_alt_text(&self) -> Option<&str> { Some("Kmer graph") }
