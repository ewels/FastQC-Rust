...
impl PerSequenceQualityScores {
    fn build_chart_svg(&self) -> Option<String> {
        ...
        let max_count = data.quality_distribution.iter().cloned().fold(0.0_f64, f64::max);
        ...
        let x_categories: Vec<String> = data.x_categories.iter().map(|v| format!("{}", v)).collect();
...
impl QCModule for PerSequenceQualityScores {
    ...
    fn chart_image_name(&self) -> Option<&str> { Some("per_sequence_quality") }
    fn chart_alt_text(&self) -> Option<&str> { Some("Per sequence quality graph") }
