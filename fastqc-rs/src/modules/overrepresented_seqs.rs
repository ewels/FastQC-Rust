...
impl std::fmt::Display for ContaminantHit {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        ...
        write!(f, "{} ({}% over {}bp)", self.name, self.percent_id, self.length)
    }
}
...
impl Contaminant {
    ...
    fn find_match(&self, query: &str) -> Option<ContaminantHit> {
        ...
        if query_upper.len() < 20 && query_upper.len() >= 8 {
            let forward_str =
                std::str::from_utf8(&self.forward).unwrap_or("");
            let reverse_str =
                std::str::from_utf8(&self.reverse).unwrap_or("");
    ...
    fn find_strand_match(
        ...
        for offset in min_offset..max_offset {
            if let Some(hit) =
                Self::find_match_at_offset(ca, cb, offset, name)
            {
