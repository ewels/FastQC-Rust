...
use super::{
    ...
    svg_footer, svg_header, svg_line, svg_rect_both, svg_rect_filled,
};
...
pub fn render_quality_boxplot(params: &QualityBoxPlotData) -> String {
    ...
    for i in 0..num_positions {
        ...
        if ugly_bottom > ugly_top {
            svg.push_str(&svg_rect_filled(x, ugly_top, base_width, ugly_bottom - ugly_top, ugly));
        }
        ...
        if bad_bottom > bad_top {
            svg.push_str(&svg_rect_filled(x, bad_top, base_width, bad_bottom - bad_top, bad));
        }
        ...
        if good_bottom > good_top {
            svg.push_str(&svg_rect_filled(x, good_top, base_width, good_bottom - good_top, good));
        }
    }
    ...
    for i in 0..num_positions {
        ...
        svg.push_str(&svg_rect_both(
            box_x + box_inset, box_top_y, box_w, box_h,
            &BOX_FILL, &black,
        ));

        // Upper whisker - vertical line from box top to whisker, horizontal cap
        svg.push_str(&svg_line(center_x, upper_whisker_y, center_x, box_top_y, &black, 1.0));
        svg.push_str(&svg_line(
            box_x + box_inset, upper_whisker_y,
            box_x + base_width - box_inset, upper_whisker_y,
            &black, 1.0,
        ));

        // Lower whisker
        svg.push_str(&svg_line(center_x, lower_whisker_y, center_x, box_bottom_y, &black, 1.0));
        svg.push_str(&svg_line(
            box_x + box_inset, lower_whisker_y,
            box_x + base_width - box_inset, lower_whisker_y,
            &black, 1.0,
        ));

        // Median line (red)
        svg.push_str(&svg_line(
            box_x + box_inset, median_y,
            box_x + base_width - box_inset, median_y,
            &MEDIAN_COLOR, 1.0,
        ));
    }
